...
# Data Set Collection
 ...
 - *Predictive Dynamics in Commodity Prices* (Gargano & Timmerman, 2012) is a loose framework for select variables therein. 
...
The VIFs of the variable reduced model are shown below. This model is retained for subsequent analysis.
...
# Indicator Variable Regression Model Fit Plot
...
# Harmonic Model Residual Assumptions

# Harmonic Model Residual Autocorrelation
...
Having achieved stationarity, at the first difference, the ACF and PACF are reviewed to determine appropriate ARIMA model parameters:
...
```{r echo = FALSE}
Box.test(arima015$residuals, lag=20)
```
...
# Stochastic Residual Corrections
...
The minimum AIC occurs at $p=3$, $q=0$. Therefore an ARIMA(3,0,0) is fit to the residuals.

```{r echo=FALSE}
kable(arima_table3 %>% arrange(arima_aic) %>% head(3),"simple")
```

# Stochastic Residual Correction Model Fit Plot
...
```{r echo = FALSE}
plot(df.ts.modelset$IIPI[424:433], col="black", xlab = "Time", ylab = "IIPI",ylim=c(100,140))
...
#  Stochastic Residual Correction Confirming White Noise
...
par(mfrow=c(1,2))
Box.test(arima210$residuals, lag=20)
Box.test(arima210$residuals^2, lag=20)
...
# ARIMA(2,1,0) with GARCH White noise confirmation
...
# ARIMA(2,1,0) with GARCH Conditional Variance
...
# ARIMA(2,1,0) with GARCH Model Fit
...
par(mfrow=c(1,2))
Box.test(arima111$residuals, lag=20)
Box.test(arima111$residuals^2, lag=20)
...
# ARIMA(1,1,1) with GARCH Model Summary
...
# ARIMA(1,1,1) with GARCH White noise confirmation
...
# ARIMA(1,1,1) with GARCH Conditional Variance
...
# ARIMA(1,1,1) with GARCH Model Fit
...
par(mfrow=c(1,2))
Box.test(mlm.ts2.arima300$residuals, lag=20)
Box.test(mlm.ts2.arima300$residuals^2, lag=20)
acf(mlm.ts2.arima300$residuals)
...
# Regression with ARMIA(3,0,0) with GARCH White noise confirmation
...
# Regression with ARMIA(3,0,0) with GARCH Model Fit
...
# Regression with ARMIA(3,0,0) with GARCH Modle Fit
...
```{r}
stats::acf(df.ts.modelset.train.vector.diff)
```

# Vector Autoregressive Moving Average CCFs
...
M2 would be a candidate for removal from the VARMA except that it is not bivariate white noise with GDP.  All others are clearly not bivariate white noise.
...
# VAR(1) Residual Assumptions

The vectors of residuals are not white noise:

```{r echo=FALSE}
par(mfrow=c(2,2))
acf(varma1_0$residuals[,1], main = "IIPI Residuals")
acf(varma1_0$residuals[,2], main = "TBillClose Residuals")
acf(varma1_0$residuals[,3], main = "GDP Residuals")
acf(varma1_0$residuals[,4], main = "InfRate Residuals")
```
...
```{r echo=FALSE}
par(mfrow=c(2,2))
acf(varma1_0$residuals[,5], main = "IndPro Residuals")
acf(varma1_0$residuals[,6], main = "M Residuals2")
acf(varma1_0$residuals[,7], main = "UnempRate Residuals")
```

# VAR(1) Model Fit of IIPI

IIPI exclusively, the model fit is nothing special when taken individually, but is useful in the generalization use case with many dependent variables of interest:

```{r echo = FALSE}

plot(df.ts.modelset$IIPI[3:433], col="light grey", xlab = "Time", ylab = "IIPI")
title("VAR(1), IIPI")
lines(c(varma1_0Ipred, pred.varma1_0.nahead), col="red")
abline(v=424,col="blue", lty=5)
```
...
# TBillClose Pre-Whitening

An ARIMA(20,0,0) is used to pre-whiten the first-differenced TBillClose, and the resulting residuals can be seen to be white noise based on the Box-Pierce test.

```{r echo = FALSE}
arimaTBillClose <- arima(df.ts.modelset.train.vector.diff$TBillClose,order = c(20,0,0))
arimaTBillClose
Box.test(arimaTBillClose$residuals, lag=20)
par(mfrow=c(1,2))
...
```{r echo = FALSE}
arimaGDP <- arima(df.ts.modelset.train.vector.diff$GDP,order = c(5,0,0))
arimaGDP
Box.test(arimaGDP$residuals, lag = 20)
par(mfrow=c(1,2))
acf(diff(df.ts.modelset.train.vector.diff$GDP, lag = 3))
pacf(diff(df.ts.modelset.train.vector.diff$GDP))
```

# InfRate Pre-Whitening
...
```{r echo = FALSE}
arimaInfRate <- arima(df.ts.modelset.train.vector.diff$InfRate,order = c(2,0,2), seasonal=list(order=c(1,0,1), period=12))
arimaInfRate
Box.test(arimaInfRate$residuals,lag=20)
par(mfrow=c(1,2))
acf(diff(df.ts.modelset.train.vector.diff$InfRate))
pacf(diff(df.ts.modelset.train.vector.diff$InfRate))
```

# IndPro Pre-Whitening 
...
arimaIndPro
Box.test(arimaIndPro$residuals,lag=20)
par(mfrow=c(1,2))
...
```{r echo = FALSE}
arimaM2 <- arima(df.ts.modelset.train.vector.diff$M2,order = c(16,0,0), seasonal=list(order=c(1,0,0), period=12))
arimaM2
Box.test(arimaM2$residuals, lag=20)
par(mfrow=c(1,2))
acf(diff(df.ts.modelset.train.vector.diff$M2))
pacf(diff(df.ts.modelset.train.vector.diff$M2))
```

# UnempRate Pre-Whitening
...
```{r echo = FALSE}
arimaUnempRate <- arima(df.ts.modelset.train.vector.diff$UnempRate,order = c(2,0,2))
arimaUnempRate
Box.test(arimaUnempRate$residuals, lag=20)
par(mfrow=c(1,2))
...
Next the CCFs are displayed to determine if any CCF indicate reasonable $b$, $r$, $s$ values:
...
# Transfer Function with IndPro

Taking IndPro as the transfer function predictor variable, and $b = 1$, $r = 1$, and $s = 0$.
...
The transfer function model of IIPI using IndPro lagged by one month as predictor with ARIMA(1,0,1) correction on the residuals results in white noise.  The model estimated is:

$IIPI_t = \frac{\omega_0}{1 - \delta_1B}IndPro_{t-1} + \epsilon_t$ 
