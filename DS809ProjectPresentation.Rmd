...
# Polynomial Model Model Fit Plot
...
# Polynomial Model Residual Assumptions
...
# Harmonic Model Model Generation
...
- pre-whitening must be performed on all the stationary first-differenced independent variables.  

- This is accomplished using various ARIMA models on each independent variable.  For each independent variable first-differenced ACF and PACF are displayed, then used for assessment of potential ARIMA model, then white noise is verified using Box-Pierce:
...
Taking IndPro as the transfer function predictor variable, and $b = 2$, $r = 1$, and $s = 0$.
