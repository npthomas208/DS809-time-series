...
```{r}
X1_b1 <- df.ts.modelset.train.vector.prewhite$IndPro[4:421]- mean(df.ts.modelset.train.vector.prewhite$IndPro)

#X2_b1 <- df.ts.modelset.train.vector.prewhite$InfRate[4:421]- mean(df.ts.modelset.train.vector.prewhite$IndPro)

X2_b4 <- df.ts.modelset.train.vector.prewhite$IndPro[1:418]- mean(df.ts.modelset.train.vector.prewhite$IndPro)

Y <- df.ts.modelset.train.vector.prewhite$IIPI[5:422] - mean(df.ts.modelset.train.vector.prewhite$IIPI)

m3<-arimax(Y, order=c(1,0,0), fixed=c(0,NA,NA,NA,NA,NA), xtransf=data.frame(X1_b1,X2_b4), transfer=list(c(1,0),c(2,0)), include.mean = FALSE)
```

```{r}
m3
acf(m3$residuals, na.action = na.pass)
pacf(m3$residuals, na.action = na.pass)
Box.test(m3$residuals)
```

```{r}
m3_arima11_resid<-arimax(Y, order=c(1,0,1), xtransf=data.frame(X1_b1,X2_b4), transfer=list(c(1,0),c(2,0)), include.mean = FALSE, method = "CSS")
m3_arima11_resid
Box.test(m3_arima11_resid$residuals,lag=20)
```
