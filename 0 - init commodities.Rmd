...
pred.arima210.garch11_1step <- pred.arima210.garch11_1step %>% append(df.ts.modelset.test$IIPI - predictions_arimagarchtemp)
...
```{r}
acf(diff(df.ts.modelset.train.vector$GDP, lag = 3))
...
```{r}
df.ts.modelset.train.vector.prewhite <- df.ts.modelset.train.vector %>%
  mutate(
...
ccf(df.ts.modelset.train.vector.prewhite$IIPI, df.ts.modelset.train.vector.prewhite$IndPro) # b = 3, r = 2, s = 7
...
```{r}
m_arima_resid<-arimax(Y, order = c(1,0,0), seasonal=list(order=c(1,0,0), period=12), xtransf=data.frame(X), transfer=list(c(2,10)), include.mean = FALSE)
m_arima_resid
Box.test(m_arima_resid$residuals)
```

```{r}
pred.tf <- (Y-m_arima_resid$residuals + mean(diff(df.ts.modelset.train$M2))) + df.ts.modelset.train$M2[2:423]
```

```{r}
plot(df.ts.modelset$M2[2:423], col="black", xlab = "Time", ylab = "M2")
title("TF for M2, with ARIMA correction on residuals")
lines(pred.tf,col="red")
abline(v=424,col="blue", lty=5)
```
...
pred.reg.arima300.garch11_1step <- pred.reg.arima300.garch11_1step %>% append(df.ts.modelset.test$IIPI - predictions_regressionarimagarchtemp)
