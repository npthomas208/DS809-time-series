...
Therefore, ARIMA(>0,1,>0)

```{r}
arima111 <- arima(df.ts.modelset$IIPI, order = c(1,1,1))
arima111
```

```{r}
arima212 <- arima(df.ts.modelset$IIPI, order = c(2,1,2))
arima212
```

```{r}
arima212 <- arima(df.ts.modelset$IIPI, order = c(1,1,2))
arima212
```

```{r}
arima212 <- arima(df.ts.modelset$IIPI, order = c(2,1,1))
arima212
```

```{r}
arima2510 <- arima(df.ts.modelset$IIPI, order = c(25,1,0))
arima2510
```
  ...
  for (p in 0:20){
    for (q in 0:20){
      if(p = 0){
        ...
        arima_aic <- arima_aic %>% append(arima(df.ts.modelset$IIPI, order = c(p,1,q))$aic)
      }
      if (q = 0){
        p_index <- p_index %>% append(p)
        q_index <- q_index %>% append(q)
