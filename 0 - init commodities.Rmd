...
pred.dlm.ts6.arima201nahead <- predict(dlm.ts6.arima201,n.ahead = 10, new = data.frame(df.ts.modelset.harmonic.test1 %>%select(-c('IIPI'))))$pred
```
...
pred.dlm.ts6.arima2011step <- predictions_armatemp %>% unlist()
...
pred.mlm.ts2.arima300nahead <- predict(mlm.ts2.arima300,n.ahead = 10, new = data.frame(df.ts.modelset.test %>%select(-c('Date_form','IIPI','GDP','IndPro'))))$pred
```
...
pred.mlm.ts2.arima3001step <- predictions_armatemp %>% unlist()
...
garch_search <- function(d){
  p_index <- list()
  q_index <- list()
  arima_aic <- list()
  for (q in 1:2){
    for (p in 0:2){
      ztemp<-garchFit(~arma(1,1)+garch(q,p),data=d, trace=FALSE) %>% summary()
      p_index <- p_index %>% append(p)
      q_index <- q_index %>% append(p)
      arima_aic <- arima_aic %>% append(ztemp@fit$ics['AIC']*length(d))
    }
  }
  return(tibble(q_index,p_index,arima_aic))
}

garch_table1 <- garch_search(d = diff(df.ts.modelset.train$IIPI))
```
