...
```{r}
X1_b1 <- df.ts.modelset.train.vector.prewhite$IndPro[3:421]- mean(df.ts.modelset.train.vector.prewhite$IndPro)

X2_b3 <- df.ts.modelset.train.vector.prewhite$M2[1:419]- mean(df.ts.modelset.train.vector.prewhite$M2)

Y <- df.ts.modelset.train.vector.prewhite$IIPI[4:422] - mean(df.ts.modelset.train.vector.prewhite$IIPI)

m3<-arimax(Y, order=c(1,0,0), fixed=c(0,NA,NA,NA), xtransf=data.frame(X1_b1,X2_b3), transfer=list(c(1,0),c(0,0)), include.mean = FALSE)
```
...
```{r}
m3_arima11_resid<-arimax(Y, order=c(2,0,2), xtransf=data.frame(X1_b1,X2_b3), transfer=list(c(1,0),c(0,0)), include.mean = FALSE)
m3_arima11_resid
