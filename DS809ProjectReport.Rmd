...
First, stationarity is induced in all variables by taking the first difference.  All pair-wise CCFs are checked for bivariate white noise.  If one variable is found to be bivariate white noise with all others, it should be removed from the subsequent VARMA model:
...
In consideration of computation time only VAR(1) model is estimated, with a matric of 49 $phi$ estimates:
...
```{r}
abs(varma1_0$coef/varma1_0$secoef)
```
...
First the variable are prepared, and a transfer function is estimated based on, b=0, r=2, s=10:
...
```{r}
multivariable_comparetable <- comparetable %>% rbind(
  data.frame(
    Model = c(
